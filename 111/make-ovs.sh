...
#Bridge mode
sudo ifconfig br-int 10.1.1.1 netmask 255.255.0.0 up

# create docker container
# sudo docker run -itd --name {name} -p {host port:container port} {image}
sudo docker run -itd --name c1 -p 8080:80 nginx
sudo docker run -itd --name c2 ubuntu:16.04

# connect bridge and container using ovs-docker
sudo ovs-docker add-port $bridge eth1 c1 --ipaddress=10.1.1.101/16 --gateway=10.1.1.1
sudo ovs-docker add-port $bridge eth1 c2 --ipaddress=10.1.1.102/16 --gateway=10.1.1.1
